            ...
            <table className="table table-striped table-hover table-fixed">
              <thead>
                <tr>
                  ...
                  <th style={{width: '42%'}}>Email / Info</th>
              ...
              <tbody>
                {items.map((it, idx) => (
                  <tr key={it.id ?? idx}>
                    ...
                    <td><small className="text-muted">{it.email ?? JSON.stringify(it).slice(0, 100)}</small></td>
                    <td>
                      <button className="btn btn-sm btn-primary me-2" onClick={() => { setSelected(it); setShowModal(true); }}>View</button>
                      <a className="btn btn-sm btn-outline-secondary" href={`mailto:${it.email ?? ''}`}>Email</a>
                    </td>
                  </tr>
                ))}
